...
sudo git clone --depth 1 --branch $OE_VERSION https://github.com/ShaheenHossain/trustedmovrs_islandaro_16ent $OE_HOME_EXT/
...
if [ $IS_ENTERPRISE = "True" ]; then
    ...
    sudo su root -c "printf 'addons_path=${OE_HOME_EXT}/islandaro/addons,${OE_HOME}/custom/addons\n' >> /etc/${OE_CONFIG}.conf"
fi
...
sudo su root -c "echo 'sudo -u $OE_USER $OE_HOME_EXT/islandaro-bin --config=/etc/${OE_CONFIG}.conf' >> $OE_HOME_EXT/start.sh"
...
DAEMON=$OE_HOME_EXT/islandaro-bin
NAME=$OE_CONFIG
DESC=$OE_CONFIG
...
server {
  ...
  # Add Headers for islandaro proxy mode
...
EOF
  ...
  sudo mv ~/islandaro /etc/nginx/sites-available/$WEBSITE_NAME
